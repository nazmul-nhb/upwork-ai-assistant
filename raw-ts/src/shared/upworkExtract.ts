...
export function extractUpworkJobFromDom(url: string): UpworkJob {
	const title = textFrom('h1') || textFrom('[data-test="job-title"]') || 'Untitled job';
	...
	const clientLocation = findLabeledValue(['Location']) || undefined;

	const clientHistorySummary = extractClientHistorySummary() || undefined;
...
function extractSkills(): string[] | undefined {
	...
	const a = chips.map((x) => normalizeSpace(x.textContent ?? '')).filter((x) => x.length > 0);
