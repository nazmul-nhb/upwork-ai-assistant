...
async function copyProposal(kind: "short" | "full"): Promise<void> {
  ...
  await navigator.clipboard.writeText(kind === "short" ? latestResult.proposalShort : latestResult.proposalFull);
}
...
async function getJobSnapshotViaBg(tabId: number): Promise<UpworkJob> {
  ...
  const [res] = await chrome.scripting.executeScript({
    ...
    func: () => {
      ...
      const title = (document.querySelector("h1")?.textContent ?? "").trim() || "Untitled job";
      const desc =
        (document.querySelector('[data-test="job-description-text"]')?.textContent ?? "").trim() ||
