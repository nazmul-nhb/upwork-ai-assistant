...
async function init(): Promise<void> {
  const res = (await chrome.runtime.sendMessage({ type: "GET_SETTINGS" } satisfies BgRequest)) as BgResponse;
  ...
  keyStatus.textContent = settings.openaiApiKey ? "Encrypted key is saved." : "No key saved yet.";
}
...
async function saveKey(): Promise<void> {
  ...
  const encrypted = await encryptSecret(apiKey, passphrase);
  const next: ExtensionSettings = {
    ...settings,
    openaiApiKey: encrypted
  };

  const res = (await chrome.runtime.sendMessage({ type: "SET_SETTINGS", settings: next } satisfies BgRequest)) as BgResponse;
  if (!res.ok) throw new Error(res.error);
