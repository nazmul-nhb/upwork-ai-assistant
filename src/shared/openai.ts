 ...
 */
export async function callOpenAiJson(apiKey: string, model: string, instructions: string, input: string): Promise<string> {
...
function extractOutputText(v: unknown): string {
  ...
  for (const item of out) {
    ...
    for (const c of content) {
      ...
      if (typeof co.type === "string" && co.type.includes("text") && typeof co.text === "string") acc += co.text;
    }
  }
  if (acc.trim().length === 0) throw new Error("Could not extract model text from OpenAI response.");
  return acc;
}
